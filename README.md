...
- **Tick-accurate Gantt chart** with merged execution blocks, idle gaps, a ruled time axis, and animated block rendering derived directly from the simulator's timeline output.
- **Aggregate metrics** — average turnaround, average waiting, average response, CPU utilization, idle-gap total, and context-switch count — computed from the same `SimulationResult` the chart consumes.
- **Per-process analysis table** — start, completion, turnaround, waiting, and response for every process in the run.
- **Cyclic preemption log** (`RrFlow`) for quantum-driven algorithms, surfacing every forced context switch directly in the workspace.
...
- **Active discipline panel**: the simulator names the current algorithm, its scheduling kind, governing mechanism, and quantum usage before you run.
- **Deep links**: `/simulator?algorithm=rr` preselects a discipline — used by the resources comparison matrix.
- An animated landing page — hero Gantt preview, algorithm marquee, animated stat counters, and a full fifteen-algorithm library grouped by family.
- A distinct dark, engineering-lab visual identity — IBM Plex Sans/Serif/Mono, a faint blueprint grid, scroll progress bar, and uppercase tracked labels throughout.
...
```
app/
├── layout.tsx              # Root layout: fonts, metadata, ambient grid backdrop, <Navigation />, <Footer />, scroll progress
├── page.tsx                # Landing page — hero + Gantt preview, stats, marquee, architecture/methodology, algorithm library, metrics, CTA
└── simulator/
    └── page.tsx            # Simulator workspace — wires state, form, table, chart, metrics, and RR preemption log together
...
└── algorithms-meta.ts      # Presentation metadata for all 15 algorithms (shared by landing, simulator, resources)

components/
├── layout/
│   ├── Navigation.tsx      # Responsive nav with mobile overlay + frosted scroll backdrop
│   ├── Footer.tsx          # Site-wide footer: brand, navigation, algorithm families, tech stack
│   └── ScrollProgress.tsx  # Reading-progress bar pinned to the top of the viewport
├── simulator/
...
└── visualizations/
    ├── GanttChart.tsx      # Timeline blocks, ruled time axis, aggregate metrics, per-process analysis
    └── RrFlow.tsx          # Cyclic preemption log for quantum-driven algorithms (context switches only)
```
...
- `lib/algorithms-meta.ts` is the single source of truth for **presentation** metadata (names, kinds, mechanisms, families) — the landing algorithm library, the simulator's active-discipline panel, and the resources comparison matrix all consume it.
...
- Deep links: the resources pages link into `/simulator?algorithm=<key>`; the workspace applies the query parameter by adjusting state during render (no effects, no hydration mismatch).
...
- Saved workspace presets (multi-process templates shared via URL).
- Complexity annotations (per-decision O(n) analysis) inside the algorithm library.
